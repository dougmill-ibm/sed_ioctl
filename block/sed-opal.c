...
static int get_pek_keyring(enum opal_key_context ctx, struct opal_key *okey)
{
	...
	if (okey->key_len == sizeof(key_serial_t)) {
		key_serial_t *id = (key_serial_t *)okey->key;
		key = key_lookup(*id);
		if (IS_ERR(key))
			return PTR_ERR(key);
		...
		ret = assoc_array_iterate(&opal_keyring->keys, keyring_iter, &key_ctx);
		if (!ret)
			return -ENOKEY;
		key = key_ctx.key;
	} else
...
static int revert_lsp(struct opal_dev *dev, void *data)
{
	...
	err = cmd_start(dev, opaluid[OPAL_THISSP_UID], opalmethod[OPAL_REVERTSP]);
	add_token_u8(&err, dev, OPAL_STARTNAME);
	add_token_u64(&err, dev, OPAL_KEEP_GLOBAL_RANGE_KEY);
	add_token_u8(&err, dev, rev->preserve_flag ? OPAL_TRUE : OPAL_FALSE);
	add_token_u8(&err, dev, OPAL_ENDNAME);
	if (err) {
					   ...
					   struct opal_session_info *opal_session)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, opal_session->who, &opal_session->opal_key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
					  ...
					  struct opal_mbr_data *opal_mbr)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, OPAL_ADMIN1, &opal_mbr->key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
			     ...
			     struct opal_mbr_done *mbr_done)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, OPAL_ADMIN1, &mbr_done->key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
				 ...
				 struct opal_shadow_mbr *info)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, OPAL_ADMIN1, &info->key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
			       ...
			       struct opal_lock_unlock *lk_unlk)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, OPAL_ADMIN1, &lk_unlk->session.opal_key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
...
static int opal_reverttper(struct opal_dev *dev, struct opal_key *opal, bool psid)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, OPAL_ADMIN1, opal);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
...
static int opal_revertlsp(struct opal_dev *dev, struct opal_revert_lsp *rev)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, OPAL_ADMIN1, &rev->key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
			    ...
			    struct opal_lock_unlock *lk_unlk)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, lk_unlk->session.who, &lk_unlk->session.opal_key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
			     ...
			     struct opal_lr_act *opal_lr_act)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, OPAL_ADMIN1, &opal_lr_act->key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
				    ...
				    struct opal_user_lr_setup *opal_lrs)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, opal_lrs->session.who, &opal_lrs->session.opal_key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
...
static int opal_set_new_pw(struct opal_dev *dev, struct opal_new_pw *opal_pw)
{
	...
	ret = opal_get_key(dev, OPAL_PREVIOUS, opal_pw->session.who, &opal_pw->session.opal_key);
	if (ret)
		return ret;
	ret = opal_get_key(dev, OPAL_CURRENT, opal_pw->new_user_pw.who, &opal_pw->new_user_pw.opal_key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
			      ...
			      struct opal_session_info *opal_session)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, OPAL_ADMIN1, &opal_session->opal_key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
					 ...
					 struct opal_read_write_table *rw_tbl)
{
	...
	ret = opal_get_key(dev, OPAL_CURRENT, OPAL_ADMIN1, &rw_tbl->key);
	if (ret)
		return ret;
	mutex_lock(&dev->dev_lock);
	setup_opal_dev(dev);
...
static int instantiate_pks_key(struct key *keyring, struct key_type *type, const char *desc)
{
	struct key *key;
	int err = 0;

	key = key_alloc(type, desc, GLOBAL_ROOT_UID, GLOBAL_ROOT_GID,
		current_cred(),
		KEY_USR_VIEW | KEY_USR_SEARCH | KEY_USR_WRITE,
		0, NULL);
	if (IS_ERR(key))
		return PTR_ERR(key);
	err = key_instantiate_and_link(key, NULL, 0, keyring, NULL);
	key_put(key);
	return err;
}
...
{
	int ret = 0;
	struct key_type *pks_key_type = NULL;

	opal_keyring = keyring_alloc(".sed_opal",
		GLOBAL_ROOT_UID, GLOBAL_ROOT_GID, current_cred(),
		(KEY_POS_ALL & ~KEY_POS_SETATTR) |
			KEY_USR_VIEW | KEY_USR_READ | KEY_USR_SEARCH | KEY_USR_WRITE,
		KEY_ALLOC_NOT_IN_QUOTA,
		NULL, NULL);
	pks_key_type = key_type_lookup(PKS_KEY_TYPE);
	if (!IS_ERR(pks_key_type)) {
		ret = instantiate_pks_key(opal_keyring, pks_key_type, DEF_CUR_PEK);
		ret = instantiate_pks_key(opal_keyring, pks_key_type, DEF_OLD_PEK);
		key_type_put(pks_key_type);
	}
	return ret;
}
